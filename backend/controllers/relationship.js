...
const getRelationships = async (req, res) => {
    const { query: { followedUserId } } = req

    const relationships = await Relationships.find({ followedUserId: followedUserId })

    res.status(200).send(relationships.map(relationship => relationship.followerUserId))
...
const deletRelationships = async (req, res) => {
    ...
    verify(token, process.env.TOKEN, async (err, userInfo) => {
        ...
        if (user.followers.includes(userId)) {
            ...
            await Relationships.findOneAndDelete({ followerUserId: userId }, { followedUserId: req.query.userId })
...
module.exports = { deletRelationships, addRelationships, getRelationships }